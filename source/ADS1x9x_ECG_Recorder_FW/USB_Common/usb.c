...
#               ifdef _MSC_                
                    if (stUsbHandle[MSC0_INTERFACE_NUMBER].edb_Index == bEndpointNumber)
                    {
...
#ifdef _MSC_
    tInputEndPointDescriptorBlock[stUsbHandle[MSC0_INTERFACE_NUMBER].edb_Index].bEPCNF &= ~(EPCNF_TOGGLE);
    tOutputEndPointDescriptorBlock[stUsbHandle[MSC0_INTERFACE_NUMBER].edb_Index].bEPCNF &= ~(EPCNF_TOGGLE);
    MscReadControl.bCurrentBufferXY = 0; //Set current buffer to X
    MscWriteControl.bCurrentBufferXY = 0; //Set current buffer to X
#endif
    usbSendZeroLengthPacketOnIEP0();
