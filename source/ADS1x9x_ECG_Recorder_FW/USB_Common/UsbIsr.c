...
#include <string.h>
//#include "..\USB_HID_API\UsbHidReportHandler.h"
...
__interrupt VOID iUsbInterruptHandler(VOID)
{
    ...
    case USBVECINT_INPUT_ENDPOINT2:
      ...
      bWakeUp = CdcToHostFromBuffer(CDC0_INTERFACE_NUMBER);
      break;
    ...
    case USBVECINT_OUTPUT_ENDPOINT2:
      ...
      if (!CdcIsReceiveInProgress(CDC0_INTERFACE_NUMBER))
      {
          if (wUsbEventMask & kUSB_dataReceivedEvent)
          {
              bWakeUp = USBCDC_handleDataReceived(CDC0_INTERFACE_NUMBER);
          }
      }
      else
      {
          //complete receive opereation - copy data to user buffer
          bWakeUp = CdcToBufferFromHost(CDC0_INTERFACE_NUMBER);
      }
      break;
