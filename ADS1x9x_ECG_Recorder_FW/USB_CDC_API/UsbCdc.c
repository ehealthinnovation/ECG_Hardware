...
} CdcWriteCtrl[CDC_NUM_INTERFACES + HID_NUM_INTERFACES];
...
} CdcReadCtrl[CDC_NUM_INTERFACES + HID_NUM_INTERFACES];
...
VOID CdcResetData()
{
    ...
    //CdcReadCtrl[intfNum].bCurrentBufferXY = X_BUFFER;
...
BYTE USBCDC_sendData(const BYTE* data, WORD size, BYTE intfNum)
{
    ...
    if (CdcWriteCtrl[intfNum].nCdcBytesToSendLeft != 0)
    {
    ...
    CdcWriteCtrl[intfNum].nCdcBytesToSend = size;
    CdcWriteCtrl[intfNum].nCdcBytesToSendLeft = size;
    CdcWriteCtrl[intfNum].pUsbBufferToSend = data;
...
BOOL CdcToHostFromBuffer(BYTE intfNum)
{
    ...
    static BYTE last_ByteSend;
   
    edbIndex = stUsbHandle[intfNum].edb_Index;
    
    if (CdcWriteCtrl[intfNum].nCdcBytesToSendLeft == 0)           // do we have somtething to send?
    {
        if (!bZeroPacketSent)               // zero packet was not yet sent
        {
            bZeroPacketSent = TRUE;

            if(last_ByteSend == EP_MAX_PACKET_SIZE_CDC)
            {
               if (CdcWriteCtrl[intfNum].bCurrentBufferXY == X_BUFFER)  
                           tInputEndPointDescriptorBlock[edbIndex].bEPBCTX = 0;
              else
                           tInputEndPointDescriptorBlock[edbIndex].bEPBCTY = 0;
              
              CdcWriteCtrl[intfNum].bCurrentBufferXY = (CdcWriteCtrl[intfNum].bCurrentBufferXY+1)&0x01; //switch buffer
            }
            
            CdcWriteCtrl[intfNum].nCdcBytesToSend = 0;   // nothing to send
    ...
    byte_count = (CdcWriteCtrl[intfNum].nCdcBytesToSendLeft > EP_MAX_PACKET_SIZE_CDC) ? EP_MAX_PACKET_SIZE_CDC : CdcWriteCtrl[intfNum].nCdcBytesToSendLeft;
    nTmp2 = *pCT1;

    if(nTmp2 & EPBCNT_NAK)
    {
        USB_TX_memcpy(pEP1, CdcWriteCtrl[intfNum].pUsbBufferToSend, byte_count); // copy data into IEP3 X or Y buffer
        *pCT1 = byte_count;                      // Set counter for usb In-Transaction
        CdcWriteCtrl[intfNum].bCurrentBufferXY = (CdcWriteCtrl[intfNum].bCurrentBufferXY+1)&0x01; //switch buffer
        CdcWriteCtrl[intfNum].nCdcBytesToSendLeft -= byte_count;
        CdcWriteCtrl[intfNum].pUsbBufferToSend += byte_count;             // move buffer pointer
        last_ByteSend = byte_count;
        
        //try to send data over second buffer
        nTmp2 = *pCT2;
        if ((CdcWriteCtrl[intfNum].nCdcBytesToSendLeft > 0) &&                                  // do we have more data to send?
            (nTmp2 & EPBCNT_NAK)) // if the second buffer is free?
        {
            // how many byte we can send over one endpoint buffer
            byte_count = (CdcWriteCtrl[intfNum].nCdcBytesToSendLeft > EP_MAX_PACKET_SIZE_CDC) ? EP_MAX_PACKET_SIZE_CDC : CdcWriteCtrl[intfNum].nCdcBytesToSendLeft;

            USB_TX_memcpy(pEP2, CdcWriteCtrl[intfNum].pUsbBufferToSend, byte_count); // copy data into IEP3 X or Y buffer
...
BYTE USBCDC_abortSend(WORD* size, BYTE intfNum)
{
    ...
    *size = (CdcWriteCtrl[intfNum].nCdcBytesToSend - CdcWriteCtrl[intfNum].nCdcBytesToSendLeft);
    CdcWriteCtrl[intfNum].nCdcBytesToSend = 0;
    CdcWriteCtrl[intfNum].nCdcBytesToSendLeft = 0;
...
VOID CopyUsbToBuff(BYTE* pEP, BYTE* pCT, BYTE intfNum)
{
    ...
    nCount = (CdcReadCtrl[intfNum].nBytesToReceiveLeft > CdcReadCtrl[intfNum].nBytesInEp) ? CdcReadCtrl[intfNum].nBytesInEp : CdcReadCtrl[intfNum].nBytesToReceiveLeft;

    USB_RX_memcpy(CdcReadCtrl[intfNum].pUserBuffer, pEP, nCount); // copy data from OEP3 X or Y buffer
    CdcReadCtrl[intfNum].nBytesToReceiveLeft -= nCount;
    CdcReadCtrl[intfNum].pUserBuffer += nCount;          // move buffer pointer
                                                // to read rest of data next time from this place

    if (nCount == CdcReadCtrl[intfNum].nBytesInEp)       // all bytes are copied from receive buffer?
    {
        //switch current buffer
        CdcReadCtrl[intfNum].bCurrentBufferXY = (CdcReadCtrl[intfNum].bCurrentBufferXY+1) &0x01;

        CdcReadCtrl[intfNum].nBytesInEp = 0;
    ...
    else
    {
        ...
        if (CdcReadCtrl[intfNum].nBytesInEp > nCount)
        {
            CdcReadCtrl[intfNum].nBytesInEp -= nCount;
            CdcReadCtrl[intfNum].pCurrentEpPos = pEP + nCount;
        }
        else
        {
            CdcReadCtrl[intfNum].nBytesInEp = 0;
...
BYTE USBCDC_receiveData(BYTE* data, WORD size, BYTE intfNum)
{
    ...
    if (CdcReadCtrl[intfNum].pUserBuffer != NULL)        // receive process already started
    {
        __bis_SR_register(bGIE); //restore interrupt status
        return kUSBCDC_intfBusyError;
    }

    CdcReadCtrl[intfNum].nBytesToReceive = size;         // bytes to receive
    CdcReadCtrl[intfNum].nBytesToReceiveLeft = size;     // left bytes to receive
    CdcReadCtrl[intfNum].pUserBuffer = data;             // set user receive buffer

    //read rest of data from buffer, if any4
    if (CdcReadCtrl[intfNum].nBytesInEp > 0)
    {
        // copy data from pEP-endpoint into User's buffer
        CopyUsbToBuff(CdcReadCtrl[intfNum].pCurrentEpPos, CdcReadCtrl[intfNum].pCT1, intfNum);

        if (CdcReadCtrl[intfNum].nBytesToReceiveLeft == 0)     // the Receive opereation is completed
        {
            CdcReadCtrl[intfNum].pUserBuffer = NULL;     // no more receiving pending
        ...
        if (CdcReadCtrl[intfNum].pCT1 == &tOutputEndPointDescriptorBlock[edbIndex].bEPBCTX)
        {
            CdcReadCtrl[intfNum].pCT1 = &tOutputEndPointDescriptorBlock[edbIndex].bEPBCTY;
            CdcReadCtrl[intfNum].pCurrentEpPos = (BYTE*)stUsbHandle[intfNum].oep_Y_Buffer;
        }
        else
        {
            CdcReadCtrl[intfNum].pCT1 = &tOutputEndPointDescriptorBlock[edbIndex].bEPBCTX;
            CdcReadCtrl[intfNum].pCurrentEpPos = (BYTE*)stUsbHandle[intfNum].oep_X_Buffer;
        }

        nTmp1 = *CdcReadCtrl[intfNum].pCT1;
        //try read data from second buffer
        if (nTmp1 & EPBCNT_NAK)                 // if the second buffer has received data?
        {
            nTmp1 = nTmp1 &0x7f;                // clear NAK bit
            CdcReadCtrl[intfNum].nBytesInEp = nTmp1;     // holds how many valid bytes in the EP buffer
            CopyUsbToBuff(CdcReadCtrl[intfNum].pCurrentEpPos, CdcReadCtrl[intfNum].pCT1, intfNum);
        }
    ...
    nTmp1 = 0;
    if (CdcReadCtrl[intfNum].bCurrentBufferXY == X_BUFFER)  //this is current buffer
    {
        if (tOutputEndPointDescriptorBlock[edbIndex].bEPBCTX & EPBCNT_NAK) //this buffer has a valid data packet
        {
            ...
            //second EP buffer
            CdcReadCtrl[intfNum].pEP2 = (BYTE*)stUsbHandle[intfNum].oep_Y_Buffer;
            CdcReadCtrl[intfNum].pCT2 = &tOutputEndPointDescriptorBlock[edbIndex].bEPBCTY;
    ...
    if (tOutputEndPointDescriptorBlock[edbIndex].bEPBCTY & EPBCNT_NAK)
    {
        ...
        //second EP buffer
        CdcReadCtrl[intfNum].pEP2 = (BYTE*)stUsbHandle[intfNum].oep_X_Buffer;
        CdcReadCtrl[intfNum].pCT2 = &tOutputEndPointDescriptorBlock[edbIndex].bEPBCTX;
        nTmp1 = 1;    //indicate that data is available
    }
    ...
    if (nTmp1)
    {
        ...
        if(nTmp1 & EPBCNT_NAK)
        {
            ...
            nTmp1 = *CdcReadCtrl[intfNum].pCT2;
            //try read data from second buffer
            if ((CdcReadCtrl[intfNum].nBytesToReceiveLeft > 0) &&       // do we have more data to send?
                (nTmp1 & EPBCNT_NAK))                 // if the second buffer has received data?
            {
                nTmp1 = nTmp1 &0x7f;                  // clear NAK bit
                CdcReadCtrl[intfNum].nBytesInEp = nTmp1;       // holds how many valid bytes in the EP buffer
                CopyUsbToBuff(CdcReadCtrl[intfNum].pEP2, CdcReadCtrl[intfNum].pCT2, intfNum);
                CdcReadCtrl[intfNum].pCT1 = CdcReadCtrl[intfNum].pCT2;
            }
        }
    }
...
BOOL CdcToBufferFromHost(BYTE intfNum)
{
    ...
    if (CdcReadCtrl[intfNum].nBytesToReceiveLeft == 0)       // do we have somtething to receive?
    {
        CdcReadCtrl[intfNum].pUserBuffer = NULL;             // no more receiving pending
        return bWakeUp;
    }
    ...
    if (CdcReadCtrl[intfNum].bCurrentBufferXY == X_BUFFER)   //X is current buffer
    {
        //this is the active EP buffer
    ...
    if(nTmp1 & EPBCNT_NAK)
    {
        ...
        CopyUsbToBuff(pEP1, CdcReadCtrl[intfNum].pCT1, intfNum);
    ...
    if (CdcReadCtrl[intfNum].nBytesToReceiveLeft == 0)     // the Receive opereation is completed
    {
        ...
        if (CdcReadCtrl[intfNum].nBytesInEp)       // Is not read data still available in the EP?
        {
...
{
    return (CdcReadCtrl[intfNum].pUserBuffer != NULL);
}
...
BYTE USBCDC_abortReceive(WORD* size, BYTE intfNum)
{
    ...
    *size = 0; //set received bytes count to 0
    ...
    if (CdcReadCtrl[intfNum].pUserBuffer)
    {
        //how many bytes are already received?
        *size = CdcReadCtrl[intfNum].nBytesToReceive - CdcReadCtrl[intfNum].nBytesToReceiveLeft;

        CdcReadCtrl[intfNum].nBytesInEp = 0;
        CdcReadCtrl[intfNum].pUserBuffer = NULL;
        CdcReadCtrl[intfNum].nBytesToReceiveLeft = 0;
    }
...
BYTE USBCDC_rejectData(BYTE intfNum)
{
    ...
    if (!CdcReadCtrl[intfNum].pUserBuffer)
    {
        ...
        CdcReadCtrl[intfNum].nBytesInEp = 0;                     // indicates that no more data available in the EP
    }
...
BYTE USBCDC_intfStatus(BYTE intfNum, WORD* bytesSent, WORD* bytesReceived)
{
    ...
    *bytesReceived = 0;
    ...
    if (CdcWriteCtrl[intfNum].nCdcBytesToSendLeft != 0)
    {
        ...
        *bytesSent = CdcWriteCtrl[intfNum].nCdcBytesToSend - CdcWriteCtrl[intfNum].nCdcBytesToSendLeft;
    }

    //Is receive operation underway?
    if (CdcReadCtrl[intfNum].pUserBuffer != NULL)
    {
        ret |= kUSBCDC_waitingForReceive;
        *bytesReceived = CdcReadCtrl[intfNum].nBytesToReceive - CdcReadCtrl[intfNum].nBytesToReceiveLeft;
    }
...
BYTE USBCDC_bytesInUSBBuffer(BYTE intfNum)
{
    ...
    if (CdcReadCtrl[intfNum].nBytesInEp > 0)         // If a RX operation is underway, part of data may was read of the OEP buffer
    {
        bTmp1 = CdcReadCtrl[intfNum].nBytesInEp;
        if (*CdcReadCtrl[intfNum].pCT2 & EPBCNT_NAK) // the next buffer has a valid data packet
        {
            bTmp1 += *CdcReadCtrl[intfNum].pCT2 & 0x7F;
        }
    }
    else
    {
